...
class TestProductModel(unittest.TestCase):
    ...
    def test_read_a_product(self):
        ...
        product = ProductFactory()
        app.logger.debug("Generated product (before create): %r", product)
        product.id = None
        product.create()
        self.assertIsNotNone(product.id)
    ...
    def test_update_a_product(self):
        ...
        product.id = None
        product.create()
        app.logger.debug("Created product (after create): %r", product)

        original_id = product.id
        new_description = "Zaktualizowany opis testowy"
        product.description = new_description

        # update in DB
        product.update()

        # id unchanged, description changed
        self.assertEqual(product.id, original_id)
        self.assertEqual(product.description, new_description)

        # only one product persists
        products = Product.all()
        self.assertEqual(len(products), 1)
